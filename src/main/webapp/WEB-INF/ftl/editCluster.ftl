...
var clusterId = window.location.pathname.replace(/\/cluster\/edit/, '').replace(/\//, '');
...
<@p.page title="编辑集群" footer=footerContent appName="editCluster">
        <form data-ng-controller="editClusterController" name="clusterForm" novalidate>
            <div>
            <legend>1、编辑集群</legend>
              ...
              <table class="table table-bordered" id="serverTable" data-container="body" data-toggle="popover" data-placement="top">
                  ...
                  <tbody>
                  	<tr data-ng-repeat="server in servers">
					    <td><input type="text" placeholder="IP" data-ng-model="server.serverIp"></td>
					    <td><input type="number" placeholder="端口" data-ng-model="server.serverPort"></td>
					    <td>
					    	<button type="button" class="btn btn-danger btn-delete btn-raised"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> 删除</button>
					    </td>
					</tr>
                  </tbody>
              </table>
...
<div class="form-group has-info">
 ...
 <div class="form-group">
      <div class="col-md-20 col-md-offset-5">
        <button type="submit" id="submit" class="btn btn-lg btn-success btn-raised"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> 保存</button>
      </div>
    </div>
</form>
</@p.page>
